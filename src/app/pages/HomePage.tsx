          ...
          {featureCards.map((card, index) => (
            <Reveal key={card.title} delay={index * 0.1}>
              <div className="group relative p-8 md:p-9 rounded-[16px] overflow-hidden transition-all duration-500 hover:shadow-lg h-full text-center" style={{ background: "#e9ecf2", boxShadow: "0 4px 12px rgba(15,23,42,0.08)" }}>
                ...
                <p className="font-sans text-[#3d3d47]" style={{ fontSize: "16px", lineHeight: 1.6, fontFamily: "Manrope, sans-serif", fontWeight: 500 }}>{card.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {howItWorksSteps.map((step, i) => (
            <Reveal key={step.title} delay={i * 0.12}>
              <div className="group overflow-hidden rounded-[16px] transition-all duration-500 hover:shadow-lg h-full flex flex-col" style={{ background: "rgba(10, 15, 25, 0.95)", backdropFilter: "blur(12px)", border: "1px solid rgba(100, 116, 139, 0.1)", boxShadow: "0 8px 24px rgba(0,0,0,0.5)" }}>
